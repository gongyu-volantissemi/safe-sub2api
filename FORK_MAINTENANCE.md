...
| `security.url_allowlist.upstream_hosts` | Fixed list of known providers | Same list + `api.deepseek.com`, `openrouter.ai` | So enabling the allowlist by default doesn't silently break the two extra hybrid-mixing providers most likely to be added. Any other custom/self-hosted provider still needs its host added manually. |
