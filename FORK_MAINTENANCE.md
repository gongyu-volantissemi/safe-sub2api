...
| `security.url_allowlist.upstream_hosts` | Fixed list of known providers | Same list + `openrouter.ai`, `api.fireworks.ai` (both US-based) | So enabling the allowlist by default doesn't silently break OpenRouter/Fireworks hybrid-mixing. `api.deepseek.com` deliberately NOT added — DeepSeek is a mainland-China-operated service (their own privacy policy discloses PRC data storage), which this fork's operator doesn't want in the default allowlist. Note `api.kimi.com`/`api.moonshot.ai`/`api.moonshot.cn` (Moonshot AI, also China-based) were already in upstream's default list and were left as-is — not something this fork changed, but worth knowing if you care about the same thing for those. Any other custom/self-hosted provider still needs its host added manually. |
