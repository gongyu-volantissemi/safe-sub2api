...
func TestGetOrCreateFingerprintRejectsMalformedUserAgentOnCreate(t *testing.T) {
	...
	require.Equal(t, defaultFingerprintValue().UserAgent, fp.UserAgent,
...
func TestDefaultFingerprintUserAgentIsAcceptable(t *testing.T) {
	require.True(t, isAcceptableFingerprintUserAgent(defaultFingerprintValue().UserAgent),
		"defaultFingerprintValue().UserAgent 必须自洽，否则自愈会陷入反复重写")
}
